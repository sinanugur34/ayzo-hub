...
export default function DogecoinIntelligenceReport({
  address,
}: {
  address: string;
}) {
    ...
    async function load() {
      ...
      try {
        ...
        if (!result.ok) {
          ...
          setError(
            result.error ||
              "Dogecoin intelligence is temporarily unavailable."
          );
      ...
      <div className="mt-6 overflow-hidden rounded-3xl border border-amber-500/20 bg-gradient-to-b from-amber-500/5 to-zinc-950/70 text-left">
        <div className="p-6 sm:p-8">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <div className="text-xs font-medium tracking-[0.18em] text-amber-300">
                AYZO DOGECOIN INTELLIGENCE
              </div>

              <h3 className="mt-2 text-xl font-semibold text-zinc-100">
                Verifying Dogecoin evidence
              </h3>

              <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-500">
                Reading bounded address history and verifying canonical
                Dogecoin transaction evidence.
              </p>
            </div>
    ...
    return (
      <div className="mt-6 rounded-3xl border border-amber-500/20 bg-amber-500/5 p-6 text-left sm:p-8">
        <div className="text-sm font-medium text-amber-300">
          Dogecoin intelligence unavailable
        </div>
      ...
      <section className="overflow-hidden rounded-3xl border border-amber-500/20 bg-gradient-to-b from-amber-500/10 via-yellow-500/5 to-zinc-950/80">
        <div className="p-6 sm:p-8">
          ...
          <div className="grid gap-3 py-6 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Stat
              label="Observed outputs"
              value={
                canonical
                  ? formatDogecoin(
                      canonical.valueKoinu
                    )
                  : "—"
              }
            />
